...
class DiscoveryAgent:
    ...
    def batch_search(
        ...
        inter_request_delay_ms: int = 200,
    ) -> list[SearchResponse]:
        """
        Execute a batch of searches, one per query string.

        Includes a configurable inter-request delay to avoid rate limiting
        on rapid sequential calls to the same API.

        Args:
            query_strings:          List of query strings to search.
            max_results:            Maximum results per query.
            inter_request_delay_ms: Milliseconds to sleep between requests.
        ...
        logger.info(f"[DiscoveryAgent] Starting batch search: {total} queries.")

        for i, query_str in enumerate(query_strings):
            logger.info(f"[DiscoveryAgent] Query [{i + 1}/{total}]: {query_str[:60]!r}")
            response = self.search(query_str, max_results=max_results)
            responses.append(response)

            # Polite delay — skip on last item
            if i < total - 1 and inter_request_delay_ms > 0:
                time.sleep(inter_request_delay_ms / 1000)

        success_count = sum(1 for r in responses if r.results)
        logger.info(
