...
# Tune these if runs are slow (raise workers) or Ollama OOMs (lower LLM workers).

MAX_LLM_WORKERS: int = max(1, int(os.environ.get("MAX_LLM_WORKERS", "4")))
MAX_SEARCH_WORKERS: int = max(1, int(os.environ.get("MAX_SEARCH_WORKERS", "8")))
TAVILY_SEARCH_DEPTH: str = os.environ.get("TAVILY_SEARCH_DEPTH", "basic")
SEARCH_INTER_REQUEST_DELAY_MS: int = max(
    0, int(os.environ.get("SEARCH_INTER_REQUEST_DELAY_MS", "0"))
)
