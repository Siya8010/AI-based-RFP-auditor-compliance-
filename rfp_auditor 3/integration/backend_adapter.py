...
────────────────────────────────
Integration Layer — Gradio ↔ Audit Engine ↔ Excel Exporter (Developer B)

This module is the clean seam between the Gradio UI and the existing backend.
It deliberately does NOT import Gradio — it only speaks in plain Python types
so it remains independently testable and reusable from other surfaces (CLI, API).
...
Integration contract for Gradio (ui/gradio_app.py):
    from integration.backend_adapter import run_full_audit

    for log_line, progress, results_table, excel_path in run_full_audit(
        pdf_path=tmp_path,
        start_page=1,
        end_page=10,
        output_dir="output",
...
def _rows_to_table(rows: list[ComplianceRow]) -> list[list[str]]:
    ...
    gr.Dataframe / gr.DataFrame display in Gradio.
...
def run_full_audit(
    ...
    yield (f"📄 PDF loaded: {pdf_file.name}", 0.02, None, None)
    logger.info(
        f"[Adapter] Starting audit — PDF: {pdf_path} | "
        f"pages: {start_page}→{end_page or 'end'} | "
        f"search_agent: {use_search_agent}"
    )

    # ── Build config ───────────────────────────────────────────────────────
    output_path = str(
        Path(output_dir) / f"audit_{pdf_file.stem}.xlsx"
    )
    ...
    if use_search_agent:
        try:
            ...
            yield ("🔍 Search agent initialised (Tavily + GoogleCSE fallback).", 0.05, None, None)
    ...
    # We estimate pipeline stage weights for the progress bar:
    #   init        2%
    #   pdf parse   15%
    #   queries     15%
    ...
    # Track search progress sub-steps
    _search_steps: list[int] = [0]  # mutable for closure
    _total_search_steps: list[int] = [1]

    # Track audit progress sub-steps
    _audit_steps: list[int] = [0]  # mutable for closure
    ...
    try:
        for log_message, pipeline_results in run_audit_pipeline(
            ...
            if "Parsing PDF" in log_message:
                ...
                # Parse total search steps for granular sub-progress
                try:
                ...
                # Parse total audit steps for granular sub-progress
                try:
                    n = int(log_message.split()[1])
            ...
            yield (log_message, round(current_progress, 3), None, None)
    ...
    if not final_results:
        yield ("⚠️  Pipeline produced no results. Check the PDF content and page range.", 0.95, None, None)
        return

    # ── Export to Excel ────────────────────────────────────────────────────
    yield (f"📊 Exporting {len(final_results)} rows to Excel...", 0.94, None, None)
    try:
        exporter = ExcelExporter()
        saved_path = exporter.export(final_results, output_path)
        yield (f"✅ Excel report saved: {saved_path}", 0.97, None, None)
    except Exception as exc:
        tb = traceback.format_exc()
        logger.error(f"[Adapter] Excel export failed:\n{tb}")
        yield (f"❌ Excel export failed: {exc}", 0.97, None, None)
        saved_path = None
