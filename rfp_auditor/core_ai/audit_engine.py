...
1. "recommended_model" CANNOT be an empty string (""). If you do not have a model recommendation or the status is "No Match", you MUST write "None" or "N/A".
2. "source_url" CANNOT be an empty string ("") and MUST begin with http:// or https://. If no web source or evidence link is available, you MUST explicitly output "https://localhost".
""".strip()
...
class AuditEngine:
    ...
    def audit_requirements(
        ...
        for i, (segment, context) in enumerate(zip(segments, web_contexts)):
            ...
            # Use cleaned_text if available (post-purifier), else fall back to raw
            text = segment.cleaned_text or segment.raw_text
            row = self.audit_single(text, context)
            results.append(row)
            # Yield progress after each segment is audited
            yield (f"Auditing segment {i + 1}/{len(segments)} (page {segment.page_number})", None)

        logger.info(f"Audit complete. {len(results)} rows produced.")
