...
class TestGeminiClientHealthCheck:
    ...
    @patch("core_ai.gemini_client.ChatGoogleGenerativeAI")
    ...
    def test_missing_api_key_raises(self, mock_chat_class):
        ...
        Patches os.environ inside config.settings (where validate_env reads live)
        rather than wiping the global environment, which avoids breaking other
        imports running in the same process.
        """
        from core_ai.gemini_client import GeminiClient
